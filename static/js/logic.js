...
function generateHeatmap(data) {
    ...
    data.forEach(function(row) {
        ...
        var state = row['ShipState'] ? row['ShipState'].trim() : ''; // Ensure state is trimmed
        ...
        if (!isNaN(lat) && !isNaN(lon) && !isNaN(count)) {
            ...
            marker.bindPopup(`<b>${state}</b><br>Count: ${count}`);
        }
    });
    ...
    L.heatLayer(heatData, {
        radius: function (point) {
            return Math.max(point[2] / 2, 5);  // Adjust radius based on count, minimum radius is 5
        },
        blur: 15,
        gradient: {
            0.0: 'blue',
